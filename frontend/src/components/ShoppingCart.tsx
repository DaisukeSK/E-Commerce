...
function ShoppingCart(){
    ...
    const removeFromCart=(productId:number,quantity:number)=>{
        axios.post('http://localhost:8080/cart/removeFromCart',{user_id:userId, product_id:productId})
        .then((res:any)=>{
    ...
    const order=()=>{
        ...
        axios.post('http://localhost:8080/history/add',{state:shoppingCart})
        .then((res:any)=>{
            
            if(res.status==200){

                axios.post('http://localhost:8080/cart/order',{user_id:userId})
                .then((res:any)=>{
    ...
    useEffect(()=>{
        axios.post('http://localhost:8080/cart/getCart',{user_id:userId})
        .then((res:any)=>{
